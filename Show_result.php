...
<body>
	<div class="container">
		...
		<h2 class =" heading">Search Result For Shows</h2>
...
$SQL = "SELECT * FROM tb_show WHERE ID Like '$term'";
  $result = mysqli_query($connection, $SQL);
  if ($result->num_rows > 0) {
     echo "<table><tr><th>ID</th><th>Start Time</th><th>End Time</th><th>Number of Shows</th></tr>";
     // output data of each row
     while($row = $result->fetch_assoc()) {
         echo "<tr><td>"  . $row["ID"]."</td><td>". $row["ST"]."</td><td>".$row["ET"]."</td><td>".$row["NS"]."</td></tr>";
     }
     echo "</table>";
} else {
     echo "0 results";
