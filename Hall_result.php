...
<body>
	<div class="container">
		...
		<h2 class =" heading">Search Result For Hall</h2>
...
$SQL = "SELECT * FROM tb_hall WHERE ID LIKE '$term'";
  $result = mysqli_query($connection, $SQL);
 if ($result->num_rows > 0) {
     echo "<table><tr><th>ID</th><th>Name</th><th>Type</th><th>Seat Number</th></tr>";
     // output data of each row
     while($row = $result->fetch_assoc()) {
         echo "<tr><td>"  . $row["ID"]."</td><td>". $row["Name"]."</td><td>".$row["Type"]."</td><td>".$row["SeatNo"]."</td></tr>";
     }
     echo "</table>";
} else {
     echo "0 results";
